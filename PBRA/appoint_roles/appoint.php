...
if ($user_type === 'admin' && $appointment_type === 'super_admin') {
    header("Location: ../appoint_roles/appoint.php?role_id=$role_id&type=admin");
    exit();
}
...
// Fetch all users based on appointment type
if ($appointment_type === 'super_admin') {
    // Super admin can appoint to regular users and admin users
    $eligible = $conn->query("
        SELECT id, full_name, email, education, work_experience, profile_pic, user_type 
        FROM users 
        WHERE id != $current_user_id AND user_type IN ('regular', 'admin')
    ")->fetch_all(MYSQLI_ASSOC);
} else {
    // Admin can only appoint to regular users
    $eligible = $conn->query("
        SELECT id, full_name, email, education, work_experience, profile_pic, user_type 
        FROM users 
        WHERE id != $current_user_id AND user_type = 'regular'
    ")->fetch_all(MYSQLI_ASSOC);
}
...
<header>
    ...
    <title><?= htmlspecialchars((string)$role['name']) ?></title>

    <body>
        <h1 style="font-size: 30px;"><?= htmlspecialchars(strtoupper((string)$role['name'])) ?></h1>
        ...
        <div class="breadcrumb">
            <ul>
                ...
                <li><a href="../appoint_roles/approle.php?type=<?= urlencode($appointment_type) ?>">
                        <?= $appointment_type === 'super_admin' ? 'Appoint Roles (super_admin)' : 'Appoint Roles (admin)' ?>
                    </a></li>
        ...
        <div class="section-title-requirements">
            ...
            <ul class="requirements">
                <?php foreach ($requirements as $req): ?>
                    <li><?= htmlspecialchars((string)$req['description']) ?></li>
                <?php endforeach; ?>
            </ul>
        </div>
            ...
            <div class="card" style="display: flex; align-items: center; gap: 20px; cursor: pointer; position: relative; margin-bottom:20px;">
                <a href="../profile/profile.php?id=<?= $row['id'] ?>" style="text-decoration: none; color: inherit; display: flex; align-items: center; flex: 1; gap: 20px;">
                    ...
                    <div>
                        ...
                        <span><?= htmlspecialchars((string)$role['name']) ?></span><br>
            ...
            ?>
                <div class="card" style="display: flex; align-items: center; gap: 20px; position: relative;">
                    ...
                    <div style="flex: 1;">
                        ...
                        <span class="user-type-badge <?= strtolower($user['user_type']) ?>"><?= htmlspecialchars(ucfirst($user['user_type'])) ?></span><br>
                        <span><strong>Education:</strong> <?= htmlspecialchars((string)$user['education']) ?></span><br>
                        <span><strong>Experience:</strong> <?= htmlspecialchars((string)$user['work_experience']) ?></span><br>
